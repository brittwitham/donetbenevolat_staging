...
def get_data():
    ...
    HealthDonorsBarriers_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-HealthDonorsBarriers_FR.csv")))
    HealthDonorsDonMeth_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-HealthDonorsDonMeth_FR.csv")))
    HealthDonorsDonRates_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-HealthDonorsDonRates_FR.csv")))
    HealthDonorsMotivations_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-HealthDonorsMotivations_FR.csv")))
    HealthVolsActivities_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-HealthVolsActivities_FR.csv")))
    HealthVolsBarriers_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-HealthVolsBarriers_FR.csv")))
    HealthVolsMotivations_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-HealthVolsReasons_FR.csv")))
    HealthVolsVolRates_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-HealthVolsVolRates.csv")))
...
def process_data(data):
    for i in range(len(data)):
        ...
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Health<br>volunteer",
            "Health volunteer",
            data[i]["Attribute"])
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Health<br>donors",
            "Health donors",
            data[i]["Attribute"])
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Non-health<br>volunteer",
            "Non-health volunteer",
            data[i]["Attribute"])
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Non-health<br>donors",
            "Non-health donors",
            data[i]["Attribute"])
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Non-health<br>donor",
            "Non-health donor",
            data[i]["Attribute"])
