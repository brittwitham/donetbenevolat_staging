...
def get_data():
    ...
    ReligionDonorsBarriers_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-ReligionDonorsBarriers_FR.csv")))
    ReligionDonorsDonMeth_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-ReligionDonorsDonMeth_FR.csv")))
    ReligionDonorsDonRates_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-ReligionDonorsDonRates_FR.csv")))
    ReligionDonorsMotivations_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-ReligionDonorsMotivations_FR.csv")))
    ReligionVolsActivities_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-ReligionVolsActivities_FR.csv")))
    ReligionVolsBarriers_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-ReligionVolsBarriers_FR.csv")))
    ReligionVolsMotivations_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-ReligionVolsReasons_FR.csv")))
    ReligionVolsVolRates_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-ReligionVolsVolRates_FR.csv")))
...
def process_data(data):
    for i in range(len(data)):
        ...
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Religion<br>volunteer",
            "Religion volunteer",
            data[i]["Attribute"])
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Non-religion<br>volunteer",
            "Non-religion volunteer",
            data[i]["Attribute"])
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Non-religion<br>donors",
            "Non-religion donors",
            data[i]["Attribute"])
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Non-religion<br>donor",
            "Non-religion donor",
            data[i]["Attribute"])
