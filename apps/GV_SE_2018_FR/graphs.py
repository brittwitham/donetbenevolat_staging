...
def triple_horizontal_rate_avg(
        dff_1,
        dff_2,
        name1,
        name2,
        ...
        title,
        giving=True):
    dff_1['Text'] = np.select([dff_1["Marker"] == "*",
                               dff_1["Marker"] == "...",
                               pd.isnull(dff_1["Marker"])],
                              [dff_1.Estimate.map(str) + "%*",
                               "...",
                               dff_1.Estimate.map(str) + "%"])
    dff_1['HoverText'] = np.select(
        [
            dff_1["Marker"] == "*",
            dff_1["Marker"] == "...",
            pd.isnull(
    ...
    if giving:
        dff_2['Text'] = np.select([dff_2["Marker"] == "*",
                                   dff_2["Marker"] == "...",
                                   pd.isnull(dff_2["Marker"])],
                                  ["$" + dff_2.Estimate.map(str) + "*",
                                   "...",
                                   "$" + dff_2.Estimate.map(str)])
        dff_2['HoverText'] = np.select(
            [
                dff_2["Marker"] == "*",
                dff_2["Marker"] == "...",
                pd.isnull(
            ...
            [
                ...
                (
                        dff_2["CI Upper"] -
                    dff_2["Estimate"]).map(str)])
        dff_1['QuestionText'] = np.select(
            [
                dff_1["QuestionText"] == 'Donor flag',
                dff_1["QuestionText"] == 'Secular donor flag',
                dff_1["QuestionText"] == 'Religious donor flag'],
            [
                "Le don en général",
                "Dons séculaires",
                "Dons religieux"])
        dff_2['QuestionText'] = np.select(
            [
                dff_2["QuestionText"] == 'Total donation amount',
                dff_2["QuestionText"] == 'Secular donation amount',
                dff_2["QuestionText"] == 'Religious donation amount'],
            [
                "Le don en général",
                "Dons séculaires",
                "Dons religieux"])
        ...
        dff_1['QuestionText'] = np.select(
            [
                dff_1["QuestionText"] == 'Volunteer flag',
                dff_1["QuestionText"] == 'Direct help flag',
                dff_1["QuestionText"] == 'Community involvement flag'],
            [
                "Volontariat",
                "Aider les autres",
        ...
        dff_2['QuestionText'] = np.select(
            [
                dff_2["QuestionText"] == 'Total formal volunteer hours',
                dff_2["QuestionText"] == 'Total hours spent helping directly',
                dff_2["QuestionText"] == 'Total hours spent on community<br>involvement'],
            [
                "Volontariat",
                "Aider les autres",
                "Engagement communautaire"])
    ...
    y2 = go.layout.YAxis(overlaying='y', side='left', autorange=False, range=[
                         0, 1.25 * max(np.concatenate([dff1["CI Upper"], dff2["CI Upper"], dff3["CI Upper"]]))])
    y1 = go.layout.YAxis(overlaying='y', side='right', autorange=False, range=[
                         0, 1.25 * max(np.concatenate([dff4["CI Upper"], dff5["CI Upper"], dff6["CI Upper"]]))])
                            ...
                            args=[
                                {
                                    "error_y": [
                                        ...
                                        dict(
                                            type="data",
                                            array=dff3["CI Upper"] - dff3["Estimate"],
                                            color="#424242",
                                            thickness=1.5),
                                        dict(
                                            type="data",
                                            array=dff4["CI Upper"] - dff4["Estimate"],
                                            color="#424242",
                                            thickness=1.5),
                                        dict(
                                            type="data",
                                            array=dff5["CI Upper"] - dff5["Estimate"],
                                            color="#424242",
                                            thickness=1.5),
                                        dict(
                                            type="data",
                                            array=dff6["CI Upper"] - dff6["Estimate"],
                                            color="#424242",
                                            thickness=1.5)],
                                    "text": [
                                        dff1['Text'],
...
def vertical_double_graph(dff, title, name1, name2, type, seniors=False):
    ...
    if seniors:
        array = dff2.sort_values(by="Estimate", ascending=False)[
            "QuestionText"]
    else:
        array = dff1.sort_values(by="Estimate", ascending=False)[
            "QuestionText"]
...
def triple_vertical_graphs_pops(dff, title, name1, name2, name3, type):
    ...
    fig.update_yaxes(
        autorange="reversed", ticklabelposition="outside", tickfont=dict(
            size=11), categoryorder='array', categoryarray=dff2.sort_values(
