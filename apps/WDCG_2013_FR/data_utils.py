...
def get_data():
    ...
    Reasons_2013 = pd.read_csv(op.abspath(
        filepath.format("2013-ReasonsForGiving_FR.csv")))
    AvgAmtReasons_2013 = pd.read_csv(op.abspath(
        filepath.format("2013-AvgAmtMotivations_FR.csv")))
    MotivationsByCause_2013 = pd.read_csv(op.abspath(
        filepath.format("2013-MotivationsByCause_FR.csv")))
...
def process_data(data):
    for i in range(len(data)):
        ...
        data[i]["Attribute"] = np.where(
            ...
            data[i]["Attribute"])
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Do not report<br>motivation",
            "Do not report motivation",
            data[i]["Attribute"])
        data[i]["Attribute"] = np.where(
            data[i]["Attribute"] == "Report<br>motivation",
            "Report motivation",
            data[i]["Attribute"])
