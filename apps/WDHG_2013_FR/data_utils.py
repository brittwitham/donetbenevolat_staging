...
def process_data_num(data_num):
    for i in range(len(data_num)):
        # Suppress Estimate and CI Upper where necessary (for visualizations
        # and error bars)
        data_num[i]["Estimate"] = np.where(
            data_num[i]["Marker"] == "...", 0, data_num[i]["Estimate"])
        data_num[i]["CI Upper"] = np.where(
            data_num[i]["Marker"] == "...", 0, data_num[i]["CI Upper"])
        ...
        data_num[i]["Group"] = np.where(
            data_num[i]["Attribute"] == "Unable to determine",
            "",
            data_num[i]["Group"])
        data_num[i]["Group"] = np.where(
            data_num[i]["Attribute"] == "Unknown", "", data_num[i]["Group"])

        data_num[i]["Attribute"] = data_num[i]["Attribute"].str.wrap(
            15, break_long_words=False)
...
def get_data():
    ...
    DonRates_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-DonRate_FR.csv")))
    AvgTotDon_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-AvgTotDon_FR.csv")))
    AvgNumCauses_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-AvgNumCauses_FR.csv")))
    FormsGiving_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-FormsGiving.csv")))
    TopCauseFocus_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-TopCauseFocus_FR.csv")))
    PropTotDon_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-PercTotDonors-updated_FR.csv")))
    # PropTotDon_2018 = pd.read_csv('tables/2018-PercTotDonors-updated.csv', encoding = 'unicode_escape', engine ='python')
    PropTotDonAmt_2018 = pd.read_csv(op.abspath(
        filepath.format("2018-PercTotDonations_FR.csv")))
    ...
    # return DonRates_2018, AvgTotDon_2018, AvgNumCauses_2018,
    # FormsGiving_2018, TopCauseFocus_2018, PropTotDon_2018,
    # PropTotDonAmt_2018
