...
def don_rate_avg_don_amt_prv(DonRates_2018, AvgTotDon_2018):
    ...
    update_dict = {'title': {'text': "Donation rate & average donation amount by province",
                             'y': 0.99},
                   'margin': {'l': 30,
                              'b': 30,
                              'r': 10,
...
def prim_cause_num_cause(dff1, dff2, name1, name2, title):
   ...
   :return: Plot.ly graph object
   '''
    # Make text column - dff1.Estimate.map(str)+"%" where Marker == NA,
    # dff1.Estimate.map(str)+"%"+"*" where Marker == *, "..." where Marker ==
    # ...
    dff1['Text'] = np.select([dff1["Marker"] == "*",
                              dff1["Marker"] == "...",
                              pd.isnull(dff1["Marker"])],
    ...
    x1 = go.layout.XAxis(overlaying='x', side='top', range=[
                         0, 1.25 * max(dff2['CI Upper'])])
    x2 = go.layout.XAxis(overlaying='x', side='bottom', range=[
                         0, 1.25 * max(dff1['CI Upper'])])
...
def fr_don_rate_avg_don_amt_prv(DonRates_2018, AvgTotDon_2018):
    ...
    fig1.update_layout(
        title={
            'text': "Taux de donateur.trice.s et montant moyen des dons par province",
            'y': 0.99},
        margin={
            'l': 30,
            'b': 30,
