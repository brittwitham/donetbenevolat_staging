...
def get_data():
    ...
    Barriers_2018 = pd.read_csv(op.abspath(
        filepath.format("2013-BarriersToGiving_FR.csv")))
    AvgAmtBarriers_2018 = pd.read_csv(op.abspath(
        filepath.format("2013-AvgAmtBarriers_FR.csv")))
    GivingConcerns_2018 = pd.read_csv(op.abspath(
        filepath.format("2013-GivingConcerns_FR.csv")))
    SolicitationConcerns_2018 = pd.read_csv(op.abspath(
        filepath.format("2013-SolicitationConcerns_FR.csv")))
    BarriersByCause_2018 = pd.read_csv(op.abspath(
        filepath.format("2013-BarriersByCause_FR.csv")))
    ...
    for i in range(len(data)):
        ...
        if not data[i]["Attribute"].isna().all():
            ...
            data[i]["Attribute"] = np.where(
                data[i]["Attribute"] == "Do not support<br>cause",
                "Do not support cause",
                data[i]["Attribute"])
            data[i]["Attribute"] = np.where(
                data[i]["Attribute"] == "Do not report<br>barrier",
                "Do not report barrier",
                data[i]["Attribute"])
            data[i]["Attribute"] = np.where(
                data[i]["Attribute"] == "Report<br>barrier",
                "Report barrier",
                data[i]["Attribute"])
