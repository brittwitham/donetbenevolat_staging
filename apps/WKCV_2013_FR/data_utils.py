...
def get_data():
    ...
    BarriersVol_2018 = pd.read_csv(op.abspath(
        filepath.format("2013-BarriersToVolunteering_FR.csv")))
    # BarriersVolMore_2018 = pd.read_csv(op.abspath(filepath.format("2013-BarriersToVolunteeringMore_FR.csv")))
    BarriersVolMore_20188 = pd.read_csv(
        'tables/2013-BarriersToVolunteeringMore-updated_fr.csv',
        encoding='unicode_escape',
        engine='python')
    BarriersVolMore_2018 = pd.read_csv(
        'tables/2013-BarriersToVolunteeringMore-updated_fr.csv')
    AvgHoursBarriersVol_2018 = pd.read_csv(op.abspath(
        filepath.format("2013-AvgHoursBarriers_FR.csv")))
    BarriersVolByCause_2018 = pd.read_csv(op.abspath(
        filepath.format("2013-BarriersVolByCause_FR.csv")))
    ...
    for i in range(len(data)):
        ...
        data[i]["CI Upper"] = np.where(
            np.isnan(
                data[i]["CI Upper"]),
            0,
            data[i]["CI Upper"]).round(0).astype(int)
