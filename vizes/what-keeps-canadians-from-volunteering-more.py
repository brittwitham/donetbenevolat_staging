...
BarriersVol_2018 = pd.read_csv("../tables/2018-BarriersToVolunteering.csv")
BarriersVolMore_2018 = pd.read_csv("../tables/2018-BarriersToVolunteeringMore.csv")
AvgHoursBarriersVol_2018 = pd.read_csv("../tables/2018-AvgHoursBarriers.csv")
BarriersVolByCause_2018 = pd.read_csv("../tables/2018-BarriersVolByCause.csv")
...
app.layout = html.Div([
    html.Div([
        html.H1('What keeps Canadians from volunteering more?')]),
    html.Div([
    ...
    html.Div([
        dcc.Graph(id='BarriersVolOverall', style={'marginTop': 50}),
        dcc.Graph(id='BarriersVolAvgHrs', style={'marginTop': 50}),
        html.Div([
            html.Div(["Select a barrier of focus:",
                      dcc.Dropdown(
                          ...
                          value='Gave enough time already',
                          style={'verticalAlgin': 'middle'}
                      ),
                      ],
                     style={'width': '50%', 'display': 'inline-block'})
        ]),
        dcc.Graph(id='BarriersVol-Gndr', style={'marginTop': 50}),
        dcc.Graph(id='BarriersVol-Age', style={'marginTop': 50}),
        dcc.Graph(id='BarriersVol-Educ', style={'marginTop': 50}),
        dcc.Graph(id='BarriersVol-Inc', style={'marginTop': 50}),
        dcc.Graph(id='BarriersVol-Relig', style={'marginTop': 50}),
        dcc.Graph(id='BarriersVol-Marstat', style={'marginTop': 50}),
        dcc.Graph(id='BarriersVol-Labour', style={'marginTop': 50}),
        dcc.Graph(id='BarriersVol-Immstat', style={'marginTop': 50}),
        html.Div([
        ...
        dcc.Graph(id='BarriersVolCauses', style={'marginTop': 50})
    ],
...
def vertical_percentage_graph_volunteers(dff, title, by="Attribute"):
    dff['Text'] = np.select([dff["Marker"] == "*", dff["Marker"] == "...", pd.isnull(dff["Marker"])],
                            [dff.Estimate.map(str) + "%" + "*", "...", dff.Estimate.map(str) + "%"])
    dff['HoverText'] = np.select([dff["Marker"] == "*",
                                  dff["Marker"] == "...",
    ...
    dff1 = dff[dff['QuestionGroup'] == "Barriers to volunteering more"]

    dff2 = dff[dff['QuestionGroup'] == "Barriers to volunteering"]

    name1 = "Volunteers"
    name2 = "Non-volunteers"
...
def update_graph(region):
    ...
    title = '{}, {}'.format("Barriers to volunteering, volunteers & non-volunteers", region)
...
def update_graph(region):
    ...
    title = '{}, {}'.format("Average hours volunteered by volunteers reporting and not reporting specific barriers", region)
...
def update_graph(region, barrier):
    ...
    title = '{}, {}'.format("Barriers to volunteering by gender", region)
...
def update_graph(region, barrier):
    ...
    dff = dff[dff["QuestionText"] == barrier]
    title = '{}, {}'.format("Barriers to volunteering by age", region)
    return vertical_percentage_graph_volunteers(dff, title)
...
def update_graph(region, barrier):
    ...
    dff = dff[dff["QuestionText"] == barrier]
    title = '{}, {}'.format("Barriers to volunteering by formal education", region)
    return vertical_percentage_graph_volunteers(dff, title)
...
def update_graph(region, barrier):
    ...
    dff = dff[dff["QuestionText"] == barrier]
    title = '{}, {}'.format("Barriers to volunteering by household income", region)
    return vertical_percentage_graph_volunteers(dff, title)

@app.callback(
    dash.dependencies.Output('BarriersVol-Relig', 'figure'),
    [
        dash.dependencies.Input('region-selection', 'value'),
        dash.dependencies.Input('barrier-selection', 'value')
...
def update_graph(region, barrier):
    ...
    dff = dff[dff["QuestionText"] == barrier]
    title = '{}, {}'.format("Barriers to volunteering by religious attendance", region)
    return vertical_percentage_graph_volunteers(dff, title)
